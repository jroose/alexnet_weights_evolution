...
from tensorflow.keras.initializers import glorot_normal, glorot_uniform, constant, identity
from tensorflow.keras.losses import SparseCategoricalCrossentropy

LocalResponseNormalization = lambda: layers.Lambda(lambda inp: tf.nn.local_response_normalization(inp, depth_radius=5, bias=2, alpha=1e-4, beta=0.75))
ImageNormalization = lambda: layers.Lambda(lambda inp: tf.image.per_image_standardization(inp))
...
def create_alexnet():
    ...
    X = Conv2D(filters=96, kernel_size=(11, 11), strides=(4, 4), padding='same', kernel_initializer=glorot_normal(), bias_initializer='zeros')(X)
    X = Activation('relu')(X)
    #X = LocalResponseNormalization()(X)
    X = MaxPooling2D((3, 3), strides=(2, 2))(X)

    # Second CL: 256 kernels of size (5, 5, 48), stride
    X = Conv2D(filters=256, kernel_size=(5, 5), strides=(1, 1), padding='same', kernel_initializer=glorot_normal(), bias_initializer='ones')(X)
    X = Activation('relu')(X)
    #X = LocalResponseNormalization()(X)
    X = MaxPooling2D((3, 3), strides=(2, 2))(X)
